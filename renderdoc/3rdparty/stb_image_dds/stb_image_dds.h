...
void stbi__dxt_decode_DXT1_block(unsigned char uncompressed[16 * 4], unsigned char compressed[8])
{
	...
	decode_colors[3] = (unsigned char)(255);
	...
	decode_colors[7] = (unsigned char)(255);
	if (c0 > c1)
	{
...
void stbi__dxt_decode_DXT23_alpha_block(unsigned char uncompressed[16 * 4], unsigned char compressed[8])
{
	...
	for (i = 3; i < 16 * 4; i += 4)
	{
		uncompressed[i] = (unsigned char)(stbi__convert_bit_range((compressed[next_bit >> 3] >> (next_bit & 7)) & 15, 4, 8));
		next_bit += 4;
	}
}
